...
private:
    ...
    //下面这组函数被process_read()调用以解析HTTP请求
    ...
    //下面这组函数process_write()调用以分析HTTP请求
    void unmap();
...
private:
    ...
    //客户端请求的目标文件的完整路径，其内容等于doc_root+m_url,doc_root是网站根目录
    ...
    char* m_file_address;
    //目标文件的状态，通过它我们可以判断文件是否存在、是否可读，并获取文件的大小等信息
